...
for index, row in df.iterrows():
    ...
    query = f"{proj_name} {City} nobroker.in"
    ...
    if "items" in data:
        for item in data["items"]:
            # if "nobroker.in" in item["link"] :
            if "nobroker.in" in item["link"] and not item["link"].startswith("https://www.nobroker.in/forum") and not item["link"].startswith("https://www.nobroker.in/blog"):
    ...
    with open(output_file, "a", newline="", encoding="utf-8") as f:
        ...
        if f.tell() == 0:  # Check if file is empty to write header
            writer.writerow(["XID", "Project Name", "City", "NoBroker.in Link"])
...
print("Results saved to 'NoBroker_links2.csv'.")
...
#     if "items" in data:
#         for item in data["items"]:
#             if "nobroker.in" in item["link"] and proj_name.replace(" ", "-").lower() in item["link"]:
#                 magic_link = item["link"]
#                 break
...
# print("Results saved to 'noBrokerUrls.csv'.")
