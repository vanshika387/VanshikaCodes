...
CSV_FILE = "gurgaon_project_details.csv"  # Replace with your actual CSV file
...
PASSWORD = "123456"  # Replace with your actual passwordxxxxxxxxxxxxxxxxxxxxxxxxx
...
for index, row in df.iterrows():
    ...
    driver.find_element(By.ID, "formBuilderId").send_keys(row.get("Builder", "NA").replace("\n", ", "))  # Handle multi-line names
    # driver.find_element(By.ID, "address").send_keys("NA")  # Address not in CSV
    address = f"{row.get('Project Name', 'NA')}, {row.get('District', 'NA')}, {row.get('State', 'NA')}"
